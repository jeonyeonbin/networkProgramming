	...
	while(1)
		...
		if(flag ==0){
			...
			if(pid>0) /* Parent Process */
			{
			     ...
			     srand(time(NULL));
		             //write(1,intro, sizeof(intro)); //1은 모니터
		             //read(0, buffer, BUF_SIZE); // 0 은키보드

			     
			while(1){
			     buffer[0] = (rand()%3)+'0';
			     printf("Select Num : %d",buffer[0]);
			...
			//	continue;
			     //}
		             result = who_win((int)buffer[0],(int)buffer[1]);
		             if (result == 0) {
			     ...
			     memset(buffer,0,BUF_SIZE); //먼저버퍼 초기화
			     str_len =read(clnt_sock, buffer, BUF_SIZE);
			     printf("buffer1 : %s\n",buffer);
			     printf("str_len: %d\n",str_len);
			    //TODO:퀵 해버리면 종료시키기 (해결해야댐)
